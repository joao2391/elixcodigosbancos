  ...
  def get_banco_by_codigo(codigo) when is_bitstring(codigo) do
    case HTTPoison.get("https://www.codigobanco.com/",
      ...
      banco =
        ...
        |> Floki.find("tbody")
        |> Floki.find("tr")
        |> Enum.map(fn {_chave, _valor1, valor2} -> %Banco{codigo: Enum.at(valor2,0) |> Floki.text(), nome: Enum.at(valor2,1) |> Floki.text(), site: Enum.at(valor2,2) |> Floki.text()} end)
  ...
  def get_banco_by_nome(nome) when is_bitstring(nome) do
    case HTTPoison.get("https://www.codigobanco.com/",
      ...
      banco =
        ...
        |> Floki.find("tbody")
        |> Floki.find("tr")
        |> Enum.map(fn {_chave, _valor1, valor2} -> %Banco{nome: Enum.at(valor2,1) |> Floki.text(), codigo: Enum.at(valor2,0) |> Floki.text(), site: Enum.at(valor2,2) |> Floki.text()} end)
